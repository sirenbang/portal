...
<html>
  <head>
    ...
    <title>角色授权</title>
    <script type="text/javascript" src="./rolePermission.js"></script>
    <style type="text/css">
    	li {
    		font-size: small;
    	}
  ...
  <body>
  	...
  	<div style="display: none;">
  		...
  		<!-- 退出form end -->
  		<!-- 翻页form start -->
  		<form action="authorizeRole" id="showUser-form">
    ...
    <!-- top start -->
		...
		<div class="ui-layout-center" style="text-align:center;">
			对 [${session.authorizeRole.name}] 授权
			<table width="100%">
				...
				<tbody>
					<s:iterator var="rolePortalGroup" value="#session.rolePortalGroups" status="st">
						<tr <s:if test="#st.odd">style="background-color:#BBBBBB"</s:if>>
							<td align="center">${st.count }</td>
							<td><span id="rolename_${rolePortalGroup.id }">${rolePortalGroup.portalGroup.name  }</span></td>
							<td> 
								<s:if test="#rolePortalGroup.permission == 1">
									已授权
								</s:if> <s:else>
									未授权
								</s:else>
							...
							<td>
								<s:if test="#rolePortalGroup.permission == 1">
									<a href="authorizeRoleAction?id=${rolePortalGroup.id }">取消授权</a>
								</s:if> <s:else>
									<a href="authorizeRoleAction?id=${rolePortalGroup.id }">授权</a>
								</s:else>
							</td>
						</tr>
					</s:iterator>
