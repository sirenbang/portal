...
public class GridPortletDescriptionDAO extends HibernateDaoSupport {
	...
	/**
	 * 	通过抽象模块描述编号查找 Link 模块描述
	 * 
	 * @param abstractId
	 * @return
	 */
	public GridPortletDescription findGridByAbstractId(int abstractId) {
		log.debug("getting GridPortletDescription instance with abstractId: " + abstractId);
		try {
			String queryStr = "frome GridPortletDescription gpd where gpd.abstractPortletDescription.id = " + abstractId;
			
			Session session = HibernateSessionFactory.getSession();
			Query query = session.createQuery(queryStr);
			List<GridPortletDescription> ipds = query.list();
			HibernateSessionFactory.closeSession();
			
			if (ipds == null || ipds.size() == 0) {
				log.debug("getting GridPortletDescription instance null with abstractId: " + abstractId);
				return null;
			} else {
				log.debug("getting GridPortletDescription instance success with abstractId: " + abstractId);
				return ipds.get(0);
			}
		} catch (RuntimeException re) {
			log.debug("getting GridPortletDescription instance failed with abstractId: " + abstractId, re);
			throw re;
		}
	}
