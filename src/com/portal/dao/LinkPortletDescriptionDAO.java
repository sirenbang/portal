...
public class LinkPortletDescriptionDAO extends HibernateDaoSupport {
	 ...
	 * @return
	 */
	public LinkPortletDescription findLinkByAbstractId(int abstractId) {
		log.debug("getting LinkPortletDescription instance with abstractId: " + abstractId);
		try {
			String queryStr = "frome LinkPortletDescription lpd where lpd.abstractPortletDescription.id = " + abstractId;
			
			Session session = HibernateSessionFactory.getSession();
			Query query = session.createQuery(queryStr);
			List<LinkPortletDescription> ipds = query.list();
			HibernateSessionFactory.closeSession();
			
			if (ipds == null || ipds.size() == 0) {
				log.debug("getting LinkPortletDescription instance null with abstractId: " + abstractId);
				return null;
			} else {
				log.debug("getting LinkPortletDescription instance success with abstractId: " + abstractId);
				return ipds.get(0);
			}
		} catch (RuntimeException re) {
			log.debug("getting LinkPortletDescription instance failed with abstractId: " + abstractId, re);
			throw re;
		}
	}
