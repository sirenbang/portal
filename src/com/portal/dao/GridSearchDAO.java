...
public class GridSearchDAO extends HibernateDaoSupport {
	...
	public List<GridSearch> findGridSearchesByPortletId(int gridId) {
		log.debug("---> finding GridSearches with portletId:" + gridId);
		try {
			String queryStr = "from GridSearch gs where gs.gridPortletDescription.id = " + gridId;
			
			Session session = HibernateSessionFactory.getSession();
			Query query = session.createQuery(queryStr);
			List<GridSearch> gses = query.list();
			HibernateSessionFactory.closeSession();
			
			log.debug("---> finding GridSearches success with portletId:" + gridId);
			return gses;
		} catch (RuntimeException re) {
			log.debug("---> finding GridSearches failed with portletId:" + gridId, re);
			throw re;
		}
	}
